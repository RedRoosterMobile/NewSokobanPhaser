...
---
// http://labs.phaser.io/edit.html?src=src\camera\game%20shader%20test%202.js
precision mediump float;
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
    ...
    vec2 p = uv + (vec2(.5)-texture2D(iChannel0, uv*0.3+vec2(iTime*0.05, iTime*0.025)).xy)*amp + (vec2(.5)-texture2D(iChannel0, uv*0.3-vec2(-iTime*0.005, iTime*0.0125)).xy)*amp;
    
	fragColor = texture2D(iChannel0, p)*tint_color;
}
void main(void)
{
    mainImage(gl_FragColor, fragCoord.xy);
